...
#define LIMIT_SWITCH_DOWN_PIN 23
#define LIMIT_SWITCH_UP_PIN   25
...
lv_obj_t *tracker_label;

static int height = 0;
static int total_height = 0;
static bool block_positive_movement = false;
static bool home_mode_active = false;
...
static void update_tracker_label() {
    lv_label_set_text_fmt(tracker_label, "T: %d mm", total_height);
    if (total_height >= MAX_HEIGHT) {
        block_positive_movement = true;
        total_height = MAX_HEIGHT;
    } else {
        block_positive_movement = false;
    }
}

static void update_height_label() {
...
static void preset1_event_cb(lv_event_t *e) {
    if (block_positive_movement) return;
...
void setup() {
    ...
    lv_obj_t *btn1 = create_button(menu_screen, "Preset 1 (2mm)", preset1_event_cb);
    ...
    lv_obj_align_to(btn3, btn1, LV_ALIGN_OUT_BOTTOM_MID, 0, 40);

    lv_obj_t *btn4 = create_button(menu_screen, "Home (0)", home_event_cb);
    lv_obj_align_to(btn4, btn3, LV_ALIGN_OUT_BOTTOM_MID, 0, 40);

    tracker_label = lv_label_create(menu_screen);
    lv_label_set_text(tracker_label, "T: 0 mm");
    lv_obj_align(tracker_label, LV_ALIGN_TOP_MID, 0, 20);
...
void loop() {
    ...
    bool limit_down = (digitalRead(LIMIT_SWITCH_DOWN_PIN) == LOW);
    bool limit_up = (digitalRead(LIMIT_SWITCH_UP_PIN) == LOW);

    if (limit_down) {
        if (home_mode_active) {
            stepper.stop();
            stepper2.stop();
            home_mode_active = false;
            total_height = -40;
            height = -40;
            update_tracker_label();
            update_height_label();
        ...
        if (is_dec_pressed) {
            ...
            stepper2.stop();
            is_dec_pressed = false;
            Serial.println("LIMIT SWITCH DOWN TRIGGERED");
        }
    }

    if (limit_up && is_inc_pressed) {
        stepper.stop();
        stepper2.stop();
        is_inc_pressed = false;
        Serial.println("LIMIT SWITCH UP TRIGGERED");
    }

    if ((is_inc_pressed || is_dec_pressed || home_mode_active) && current_time - last_step_time >= step_interval) {
        if (is_inc_pressed && !limit_up && !block_positive_movement) {
        ...
        if (home_mode_active && !limit_down) {
            moveStepper(0.05 * STEPS_PER_MM, LOW);
            total_height--;
            update_tracker_label();
            last_step_time = current_time;
